...
public class InvoiceService {
    ...
    // ── Brand colours ────────────────────────────────────────────
    private static final DeviceRgb RED_AVO_RED   = new DeviceRgb(0xC0, 0x39, 0x2B);
    ...
    private static final String BUSINESS_PHONE   = "+263 77 123 4567";
    private static final String BUSINESS_EMAIL   = "hello@redavo.co.zw";
    private static final String BUSINESS_WEBSITE = "www.redavo.co.zw";
    ...
    public byte[] generateInvoice(Order order) {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            bizCell.add(new Paragraph("RED AVO")
                    .setFont(bold).setFontSize(22).setFontColor(RED_AVO_RED).setMarginBottom(1));
            bizCell.add(new Paragraph("Authentic · Fearless")
                    .setFont(regular).setFontSize(8).setFontColor(DARK_RED).setMarginBottom(5));
            ...
            invTypeCell.add(new Paragraph("FISCAL TAX INVOICE")
                    .setFont(bold).setFontSize(14).setFontColor(RED_AVO_RED).setMarginBottom(3));
            invTypeCell.add(new Paragraph(order.getInvoiceNumber() != null ? order.getInvoiceNumber() : "INV-" + order.getId())
            ...
            document.add(new LineSeparator(new com.itextpdf.kernel.pdf.canvas.draw.SolidLine(1.5f))
                    .setStrokeColor(RED_AVO_RED).setMarginBottom(8));
            ...
            custCell.add(new Paragraph("BILL TO").setFont(bold).setFontSize(8)
                    .setFontColor(RED_AVO_RED).setMarginBottom(3));
            custCell.add(new Paragraph(order.getCustomerName() != null ? order.getCustomerName() : "Walk-in Customer")
            ...
            payCell.add(new Paragraph("PAYMENT").setFont(bold).setFontSize(8)
                    .setFontColor(RED_AVO_RED).setMarginBottom(3));
            payCell.add(new Paragraph("Method: " + (order.getPaymentMethod() != null ? order.getPaymentMethod() : "CASH"))
            ...
            if (order.getAmountTendered() != null && order.getAmountTendered() > 0) {
                ...
                payCell.add(new Paragraph(String.format("Change:   $%.2f", order.getChangeGiven() != null ? order.getChangeGiven() : 0))
                        .setFont(regular).setFontSize(9));
            }
            ...
            //  LINE ITEMS TABLE
            // ══════════════════════════════════════════════════════
            Table itemsTable = new Table(UnitValue.createPercentArray(new float[]{35, 8, 16, 16, 12, 13}))
                    .setWidth(UnitValue.createPercentValue(100))
                    .setMarginBottom(4);
            ...
            for (String h : headers) {
                itemsTable.addHeaderCell(
                        new Cell().add(new Paragraph(h).setFont(bold).setFontSize(8).setFontColor(ColorConstants.WHITE))
            ...
            if (order.getItems() != null) {
                ...
                for (OrderItem item : order.getItems()) {
                    ...
                    double vatRate     = 15.0;
                    double exclVat     = unitPrice * qty / (1 + vatRate / 100);
                    double vatOnItem   = unitPrice * qty - exclVat;
            ...
            // Totals
            addTotalRow(totalsTable, regular, bold, "Subtotal (excl. VAT):", String.format("$%.2f", totalExclVat), false);
            addTotalRow(totalsTable, regular, bold, "VAT @ 15%:", String.format("$%.2f", totalVat), false);

            // Grand total with red background
            Cell labelCell = new Cell().add(new Paragraph("TOTAL DUE (incl. VAT):")
                    ...
                    .setBackgroundColor(DARK_RED).setBorder(Border.NO_BORDER)
            ...
            String[] vatHeaders = {"VAT CODE", "RATE", "EXCL. VAT", "VAT AMT", "INCL. VAT"};
            ...
            String customerFirst = order.getCustomerName() != null
                    ? order.getCustomerName().split(" ")[0] : "Valued Customer";

            document.add(new Paragraph("Hi " + customerFirst + "! \uD83E\uDD51\u2764\uFE0F  Thank you for your Red Avo purchase! "
                    + "Move with confidence — The Red Avo Team")
                    .setFont(bold).setFontSize(9).setFontColor(RED_AVO_RED)
            ...
            document.add(new Paragraph(BUSINESS_WEBSITE + "  |  " + BUSINESS_EMAIL + "  |  " + BUSINESS_PHONE)
                    .setFont(regular).setFontSize(7).setFontColor(ColorConstants.GRAY)
                    .setTextAlignment(TextAlignment.CENTER));
